...
class ResetPasswordController extends AbstractController
{
    ...
    public function request(Request $request, MailerInterface $mailer, TranslatorInterface $translator, UrlGeneratorInterface $urlGenerator): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            return $this->processSendingPasswordResetEmail(
                $form->get('addEmail')->getData(),
                $mailer,
                $translator,
                $urlGenerator // Pass the UrlGeneratorInterface instance
            );
        }
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer, TranslatorInterface $translator, UrlGeneratorInterface $urlGenerator): RedirectResponse
    {
        ...
        // Generate the reset link using the route name and any route parameters
        $resetLink = $urlGenerator->generate('app_reset_password', ['token' => $resetToken], UrlGeneratorInterface::ABSOLUTE_URL);
    
        $email = (new TemplatedEmail())
            ->from(new Address('no-reply@powerfit.com', 'MailBot'))
            ->to($user->getAddEmail())
            ->subject('Your password reset request')
            ->htmlTemplate('security/email.html.twig')
            ->context([
                'resetToken' => $resetToken,
                'resetLink' => $resetLink, // Pass the reset link to the email template
            ]);
